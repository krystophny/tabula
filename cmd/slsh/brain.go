...
func isBrainSubcommand(args []string) bool {
	...
	for i < len(args) && strings.HasPrefix(args[i], "-") {
		...
		// Skip flag values: next arg is the value if current is --flag or -flag and next doesn't start with -
	...
	// "brain" alone is also a brain command (shows help).
	if i+1 >= len(args) {
		return true
	}
...
func splitBrainArgs(args []string) ([]string, []string) {
	...
	// Return args starting from subcommand (skip "brain") + rest after subcommand.
	if subIdx >= 0 {
		return args[subIdx:], args[subIdx+1:]
	}
	// No subcommand, return ["brain"] with empty rest.
	return args[brainIdx:], nil
}
