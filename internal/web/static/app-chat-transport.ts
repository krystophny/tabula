...
export function buildCanvasPositionPayload(anchor, options: Record<string, any> = {}) {
  ...
  const visual = captureVisualReasoningContext(
    Number(options?.clientX ?? anchor?.x),
    Number(options?.clientY ?? anchor?.y),
  );
  if (visual?.snapshotDataURL) {
    payload.snapshot_data_url = visual.snapshotDataURL;
  }
