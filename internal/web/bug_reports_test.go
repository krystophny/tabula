...
func TestHandleBugReportCreateWritesBundleUnderWorkspaceArtifacts(t *testing.T) {
	...
	workspace, err := app.store.CreateWorkspace("Slopshell", workspaceDir, store.SphereWork)
	if err != nil {
		...
		"meeting_diagnostics": map[string]any{
			...
			"participant_status": map[string]any{
				"decision_summary": map[string]any{
					"pickup":  "Not picked up because the latest request did not address Slopshell and did not match the tracked speaker.",
	...
	if got := strFromAny(bundle["active_workspace"]); got != "Slopshell" {
		t.Fatalf("active_workspace = %q, want %q", got, "Slopshell")
	}
...
func TestBugReportIssueBodyOmitsLocalPathsAndIncludesDiagnostics(t *testing.T) {
	...
	bundle := bugReportBundle{
		...
		ScreenshotPath:      ".slopshell/artifacts/bugs/20260311-110721-568aa357/screenshot.png",
...
func initGitRepo(t *testing.T, dir string) {
	...
	commands := [][]string{
		{"git", "init"},
		{"git", "config", "user.email", "slopshell@example.com"},
		{"git", "config", "user.name", "Slopshell Test"},
	}
