...
func (a *App) handleBugReportCreate(w http.ResponseWriter, r *http.Request) {
	...
	bundle := bugReportBundle{
		...
		Version:             firstNonEmpty(strings.TrimSpace(req.Version), slopshellVersion),
...
func (a *App) resolveBugReportWorkspace() (bugReportWorkspace, error) {
	...
	if workspace, ok, err := a.resolveSlopshellBugReportWorkspace(); err != nil {
		return bugReportWorkspace{}, err
	} else if ok {
		return workspace, nil
	}
...
func (a *App) createGitHubIssueFromBugReport(workspace bugReportWorkspace, bundlePath string, bundle bugReportBundle) (ghIssueListItem, int64, error) {
	...
	if err := a.ensureGitHubLabels(githubCWD, slopshellBugReportOwnerRepo, map[string]struct {
		Color       string
		Description string
	}{
	...
	item, err := a.store.CreateItem(strings.TrimSpace(issue.Title), store.ItemOptions{
		...
		SourceRef:   optionalTrimmedString(githubIssueSourceRef(slopshellBugReportOwnerRepo, issue.Number)),
	})
	if err != nil {
		return ghIssueListItem{}, 0, err
	}
	if err := a.syncGitHubIssueArtifact(item, slopshellBugReportOwnerRepo, issue); err != nil {
		return ghIssueListItem{}, 0, err
	}
...
func (a *App) resolveSlopshellBugReportWorkspace() (bugReportWorkspace, bool, error) {
	workspaceID, err := a.store.FindWorkspaceByGitRemote(slopshellBugReportOwnerRepo)
	if err != nil {
	...
	repoRoot := resolveCanonicalGitHubRepoRoot(slopshellBugReportOwnerRepo)
	if repoRoot == "" {
...
func resolveBugReportGitHubCommandDir(workspaceDir string) string {
	if repoRoot := resolveCanonicalGitHubRepoRoot(slopshellBugReportOwnerRepo); repoRoot != "" {
		return repoRoot
	}
...
func createBugReportDir(workspaceDir, rawTimestamp string) (string, string, error) {
	...
	dir := filepath.Join(workspaceDir, ".slopshell", "artifacts", "bugs", reportID)
