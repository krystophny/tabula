...
func TestDistillReviewedExamplesBuildsBoundedSummary(t *testing.T) {
	...
	if training.ReviewCount != 6 {
		t.Fatalf("ReviewCount = %d, want 6", training.ReviewCount)
	}
	...
	if !strings.Contains(joined, "Manual review distribution: keep=2, archive=1, trash=3") {
