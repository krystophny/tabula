...
func InterfacesMarkdown() string {
	...
	b.WriteString("This document summarizes the current Slopshell HTTP and private runtime interfaces.\n\n")
	b.WriteString("There are exactly two external agent-facing MCP servers in the sloppy stack: `sloppy` (`sloptools mcp-server`) and `helpy` (`helpy mcp-stdio`). Slopshell itself is UI/runtime only. The routes and tool names listed here describe Slopshell's private runtime/control surface, not a third external AI server.\n\n")
