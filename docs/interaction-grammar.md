...
This document is the canonical product reference for Slopshell's interaction model.
...
| Code workspace (`~/code/tabula/`) | One workspace per repo | GitHub issues/PRs synced as artifacts | Issues/PRs are items tracked here | work/slopshell, work/plasma-codes |
...
The canvas runtime exposes one persistent control surface: the Slopshell Circle.
...
Slopshell uses a layered source of truth for UI behavior across web, iOS, and Android:

- Component contract: `internal/web/static/slopshell-circle-contract.ts`
...
Slopshell may accept input through these modalities:
