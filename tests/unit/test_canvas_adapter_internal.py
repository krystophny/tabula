...
def test_launch_canvas_background_uses_expected_command(monkeypatch, tmp_path: Path) -> None:
    ...
    proc = launch_canvas_background(tmp_path, tmp_path / "events.jsonl", poll_interval_ms=321)
...
def test_events_path_property_returns_resolved_path(tmp_path: Path) -> None:
    events = tmp_path / "events.jsonl"
    adapter = CanvasAdapter(project_dir=tmp_path, events_path=events, headless=True, start_canvas=False)
    assert adapter.events_path == events.resolve()
...
def test_canvas_process_launch_and_reuse(monkeypatch, tmp_path: Path) -> None:
    ...
    # Mark previous process as terminated; next activation should relaunch.
